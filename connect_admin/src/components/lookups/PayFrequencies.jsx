...
import { CallPayFrequencies } from '../../api-call/api';

const PayFrequenciesLookup = () => {
    ...
    // Get Pay frequencies Api
    const GetPayFrequencies = async () => {
        try {
            const {response, jsonResponse, error} = await CallPayFrequencies();
            if (error) {
                setGetItemsError(error)
                return;
            }
            if (response.ok) {
                setPayFrequencies(jsonResponse.data);
                return
            } else {
    ...
    // Handle pay-frequencies menu toggle
    const togglePayFrequenciesMenu = (index) => {
                ...
                <div className='bg-white mt-4 mb-5 px-3 rounded-lg lookupsContiner'>
                    <div className="flex items-center">
                        <div className="flex items-start ps-1 pt-3 w-full lookupContainerListTittleDiv">
                            <h2 className='pe-2 pb-2 w-full font-semibold text-gray-700'>5. Pay Frequencies</h2>
                    ...
                    <div className="" id='cities'>
                        <div>
                            {payFrequencies.length > 0 ?
                                payFrequencies.map((element, index) => (
                                    <div className="pt-3">
                                        <div className="flex items-center p-2 border-b border-b-gray-300 rounded w-full">
                                            ...
                                            {/* <div className='flex justify-end lookupActionIconDiv'>
                                                <CiMenuKebab onClick={() => togglePayFrequenciesMenu(index)} className='text-black text-xl cursor-pointer' />
                                            </div> */}
...
export default PayFrequenciesLookup