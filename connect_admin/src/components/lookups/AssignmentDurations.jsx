...
import { CallAssignmentDurations } from '../../api-call/api';
...
const AssignmentDurations = () => {
    ...
    // Get assignment durations Api
    const GetAssignmentDurations = async () => {
        try {
            const { response, jsonResponse, error } = await CallAssignmentDurations();
            ...
            if (response.ok) {
                setDurations(jsonResponse.data);
                return
            } else {
    ...
    // Handle assignment durations menu toggle
    const toggleDurationsMenu = (index) => {
        setShowSubMenu(prevIndex => (prevIndex === index ? null : index));
    };


    useEffect(() => {
        GetAssignmentDurations()
    }, []);
                ...
                <div className='bg-white mt-4 mb-5 px-3 rounded-lg lookupsContiner'>
                    <div className="flex items-center">
                        <div className="flex items-start ps-1 pt-3 w-full lookupContainerListTittleDiv">
                            <h2 className='pe-2 pb-2 w-full font-semibold text-gray-700'>11. Assignment Durations</h2>
                                ...
                                durations.map((element, index) => (
                                    <div key={index} className="pt-3">
                                        <div className="flex items-center p-2 border-b border-b-gray-300 rounded w-full">
                                            ...
                                            {/* menu icon */}
                                            {/* <div className='flex justify-end lookupActionIconDiv'>
                                                <CiMenuKebab onClick={() => toggleDurationsMenu(index)} className='text-black text-xl cursor-pointer' />
                                            </div> */}
...
export default AssignmentDurations