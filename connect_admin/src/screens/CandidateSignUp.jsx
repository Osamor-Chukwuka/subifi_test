...
import { CandidateChartApiCall } from '../api-call/api';
...
const CandidateSignUp = () => {
  ...
  const getCandidateChart = async (startDate = null, endDate = null) => {
    let currentDate;
    let monthAgo;
    ...
    try {
      const { response, jsonResponse, error } = await CandidateChartApiCall(startDateTime, endDateTime);
  ...
  const handleDateChange = (update) => {
    ...
    if (update[0] != null && update[1] != null) {
      getCandidateChart(update[0], update[1])
  ...
  return (
    <div className="bg-white shadow-md p-6 rounded-lg">
      <h2 className="mb-4 font-bold text-slate-700 text-lg">Candidate Sign Ups</h2>
      ...
      <div className="flex justify-between items-center mb-4">
        <p className='w-44 text-gray-500 text-md'>Total Candidates account created</p>
        <p className="items-center text-green-500">
          <p className='font-semibold text-2xl chartDate'>{sum}</p>
          {/* <p className="text-red-400 text-xs">-2.00% ⬇</p> */}
        </p>
      </div>
      <ResponsiveContainer width="100%" height={250}>
        {error ?
          ...
          <BarChart data={data} >
            ...
            <Tooltip />
            <Legend />
            <Bar dataKey="value" fill="#FF9696" barSize={30} radius={5} />
          </BarChart>
        }
      </ResponsiveContainer>
    </div>
